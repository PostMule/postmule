...
def _build_config_yaml(
    ...
    """Return a populated config.yaml string from user-supplied installer values."""
    safe_dir = str(install_dir).replace("\\", "/")
    return f"""\
# PostMule configuration — generated by installer wizard
app:
  name: PostMule
  version: "0.1.0"
...
notifications:
  ...
  forward_to_me_urgent: true
  bill_due_alert_days: 7

mailbox:
  providers:
    - service: {vpm_provider}
      enabled: true
      scan_sender: "{vpm_sender or 'noreply@virtualpostmail.com'}"
      scan_subject_prefix: "{vpm_prefix or '[Scan Request]'}"
...
data_protection:
  soft_deletes_only: true
  trash_retention_days: 90
  max_files_moved_per_run: 50
