...
log = logging.getLogger("postmule.llm.openai")
...
class OpenAIProvider:
    ...
    Args:
        api_key:      OpenAI API key from credentials.
    ...
    def health_check(self):
        """Return a HealthResult indicating whether the OpenAI API key is valid."""
        from postmule.providers import HealthResult
        try:
            import openai  # type: ignore[import]
            client = openai.OpenAI(api_key=self.api_key)
            client.models.list()
            return HealthResult(ok=True, status="ok", message=f"OpenAI connected ({self.model_name})")
    ...
    def classify(
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenAI classification failed: {exc}\n"
